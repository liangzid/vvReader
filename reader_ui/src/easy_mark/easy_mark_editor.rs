...
impl EasyMarkEditor {
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui,fname:&mut String) {
		...
		"zh"=>"渲染界面",_=>"Show rendered",
            ...
            ui.checkbox(&mut self.show_rendered, tth);
	    let tth=match self.lang.as_str(){
		"zh"=>"语法高亮",_=>"Highlight editor",
	    };
            ui.checkbox(&mut self.highlight_editor, tth);
...
fn nested_hotkeys_ui(&self, ui: &mut egui::Ui) {
	...
	if self.lang.as_str()=="zh"{
        label(SHORTCUT_BOLD, "*加粗*");
        label(SHORTCUT_CODE, "`代码`");
        label(SHORTCUT_ITALICS, "/斜体强调/");
        label(SHORTCUT_SUPERSCRIPT, "^上标^");
        label(SHORTCUT_STRIKETHROUGH, "~删除线~");
        label(SHORTCUT_UNDERLINE, "_下划线_");
	}
	else{
...
# Example Document
This part of code is based on `EasyMark`, a markup language editor, designed for extreme simplicity.

It's github link: [egui:easy_mark](https://github.com/emilk/egui/tree/master/crates/egui_demo_lib/src/easy_mark)

I modified this editor and change somethings for VVReader.

Here is the original document to help use note as a markdown style notes.
